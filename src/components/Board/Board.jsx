...
const Board = () => {
  ...
  const isOutOfBorder = (head) => {
    if (head[0] >= 360 || head[1] >= 320 || head[0] < 0 || head[1] < 0) {
      return true;
    } else {
      return false;
    }
  ...
  const addItem = ({ getter, setter }) => {
    ...
    if (coordinates.x < 0 || coordinates.x >= 360 || coordinates.y < 0 || coordinates.y >= 320) {
      console.error("Coordinates out of bounds:", coordinates);
      return;
    }
